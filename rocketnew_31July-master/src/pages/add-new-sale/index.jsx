  ...
  const handleSubmit = async (saveAsDraft = false) => {
    ...
    try {
      const saleData = {
        ...
        accessories_value: accessories.reduce((sum, item) => sum + (parseFloat(item.price) || 0), 0),
        warranty_selling_price: warranties.reduce((sum, item) => sum + (parseFloat(item.sellingPrice) || 0), 0),
        warranty_cost: warranties.reduce((sum, item) => sum + (parseFloat(item.cost) || 0), 0),
        service_price: maintenanceItems.reduce((sum, item) => sum + (parseFloat(item.price) || 0), 0),
        service_cost: maintenanceItems.reduce((sum, item) => sum + (parseFloat(item.cost) || 0), 0),
        spiff_bonus: spiffs.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0),
