...
const SalesGrid = () => {
  ...
  // Load commissions-driven data from Supabase (per-user attribution: Option 2)
  useEffect(() => {
    let isMounted = true;
    ...
    const loadCommissionEntries = async () => {
      ...
      try {
        ...
        // Members: only own entries; Admins: pass null to fetch all
        const result = await salesService.getUserCommissionEntries(isAdmin ? null : user.id);

        if (!isMounted) return;

        if (result?.success) {
          // Map commission entries to grid row shape
          const rows = (result?.data || []).map((entry) => ({
            id: entry?.sale_id, // keep sale id for navigation/edit
            date: entry?.sale_date || entry?.created_at?.split('T')?.[0],
            customerName: entry?.customer_name || 'Unknown',
            vehicleDetails: `${entry?.vehicle_type === 'new' ? 'New' : 'Used'} Vehicle`,
            stockNumber: entry?.stock_number || '',
            salePrice: 0, // optional: can fetch sale price with additional select if needed
            commissionAmount: parseFloat(entry?.amount || 0),
            status: entry?.status === 'completed' ? 'Completed' :
                   entry?.status === 'pending' ? 'Pending' : 'Cancelled',
            salesperson: entry?.user_name || '', // for admin view, shows the commission recipient
          ...
          setError(result?.error || 'Failed to load commission entries');
        }
      } catch (error) {
        if (isMounted) {
          setError('Failed to load commission entries. Please try again.');
          console.log('Error loading commission entries:', error);
        }
      } finally {
        if (isMounted) {
          setLoading(false);
